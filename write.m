...
filename = './2021-05-27_144551.aris';
%fids = fopen('001.txt','w');
...
fclose(fid);
% close the file and reopen, reading frame
fid = fopen(filename,'rb');
fseek(fid,1024,'bof');
for i=1:1:numframes
...
a  = fread(fid,292,'uint8');
b = fread(fid,numbeams*samplesperbeam,'uint8');  % read image in b
c=reshape(b,[numbeams,samplesperbeam]);
c=c';
c=c/255;
%imshow(c,[0,255]);
imshow(c);  % change here to save the image
%binary_image_name=strcat('./image3/im_',num2str(i),'.jpg'); 
%imwrite(c,binary_image_name,'jpg');
%fprintf(fids,'%f\t%f\t%f\n',-1*compassroll/180*pi,compasspitch*-1/180*pi,compassheading/180*pi);
end
fclose(fid);